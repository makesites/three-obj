...
Main.prototype = {
	...
	load: function ( file, callback ) { 
		...
		fs.readFile(file, 'utf8', function( err, obj ){
			...
			var response = scope.parse( obj );
			// calculate vertices
			/*
			var vertices = [];
			for( var i in response.children ){
				var group = response.children[i];
				//children
				group.geometry.vertices.map(function( point ){
			...
			//",".join(generate_vertex(v, TRUNCATE, SCALE) for v in vertices)
